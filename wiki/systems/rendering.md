...
- `client/rendering/backdrop.gdshader` (M1): per-system space backdrop. Nebula color comes from a deterministic, aspect-corrected square crop of `assets/backgrounds/galaxy_full.jpg` picked by galaxy-map position (core systems on the bright bulge); crisp detail comes from two procedural star layers hashed from world position, because no crop of a 7200px master survives gameplay zoom sharp. The plane is small (240 units) to keep texture density up.
- `client/rendering/body_billboard.gdshader` (M1): camera-facing quad that plays rotation sheets (`frames` > 1 at `fps`) or animates single stills by UV rotation; suns add a counter-rotating screen-blended layer and a scale pulse; biome/star tints recolor luminance by `tint_mix`. See `assets/README.md` Bodies pipeline.

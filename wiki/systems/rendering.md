...
- `client/rendering/system_view.gd` (M1): builds the current system from `StarSystem` data — a backdrop plane sampling a deterministic per-system crop of `assets/backgrounds/galaxy_full.jpg` (galaxy-map position → image UV, so core systems sit on the bright bulge), billboard suns and planets from `data/bodies/*.json` via `body_catalog.gd`, and primitive station boxes / gate tori with `Label3D` names until the art pass.

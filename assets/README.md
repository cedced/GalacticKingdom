...
- Implemented at M1 **without pre-cropped files**: `SystemView` samples the master directly with per-system UV offsets, mapping the system's galaxy-map position onto the image — a core system gets the bright bulge, a rim system a dark arm, deterministically, with zero generated files. This supersedes the planned `tools/backdrop_crop.py` + `backdrop_mapping.json`; resurrect that pipeline only if the runtime approach fails somewhere (see Open questions on export size).
- If pre-cropped files ever return: generated by tool, never by hand, 2048x2048 max, deterministic from seed + system id.
...
- The 23 MB backdrop master is now referenced at runtime, so it would ship in exported builds. Revisit (downscaled copy for export, or pre-crops) when the export pipeline exists.
...
- [x] Per-system backdrops (M1: runtime UV crop of the master; `backdrop_crop.py` superseded, see Backgrounds pipeline)
